...
author:
  - Edward Wallace^[University of Edinburgh, jane@example.org]
  - Xuejia Ke^[University of Edinburgh, edward.wallace@ed.ac.uk]
date: "July 2020"
output:
  html_document:
    toc: true
...
![Experiment example design](system.file("vignettes", "Experiment_design.png", package = "RNAFracQuant"))
...
## 1.2 Example data
This vignette builds from a simulated dataset with two replicates, two fractions and six sample count files. For each count file, there are twenty transctipts, named from G1 to G20. Count values for Total, Pellet, Supernatant and pSup values for transcripts are simulated as below:

$pSup: \mathcal{U}(min=0,max=1)$  
$scaling.factor.Pellet: \mathcal{U}(min=0,max=1)$  
$scaling.factor.Sup: \mathcal{U}(min=0,max=1)$  
$N_{total}: X\sim\mathcal{N}(\mu,\,\sigma^{2})$  
...
ORF <- paste0("G",1:20)
set.seed(5)
Tot1 <- floor(rlnorm(20, log(8000), log(1.5)))
...
$Tot=scaling.factor.Sup \times Sup + scaling.factor.Pellet \times Pellet$  
...
In function **model_fit**, the number of chains in sampling is default to 4, the number of iteration is default to 1000 and the control is default to "list(adapt_delta = 0.85)". You can specify those values as you prefer. For example, `model_fit(wide_data = mydata, chains = 3, iter = 1500, control = list(adapt_delta = 0.85))`. Those values can also be specified in function **get_paras_median**. See [rstan::sampling](https://mc-stan.org/rstan/reference/stanmodel-method-sampling.html) for more information. If you are interested in the stan model evaluation, you can view this bayesian statistical model graphical diagnosis by `shinystan::launch_shinystan(model_fit(wide_data))`. See [launch_shinystan](https://www.rdocumentation.org/packages/rstanarm/versions/2.15.3/topics/shinystan) or type `?shinystan::launch_shinystan` in Rstudio for more info.
...
Get the estimated medians of parameters for each condition and output them in a single data frame. Similar to function **model_fit**, you can also specify the values of "chains", "iteration" and "control" as you prefer. For example, `get_paras_median(wide_data = mydata, chains = 3, iter = 1500, control = list(adapt_delta = 0.85))`.
```{r get_paras_medians, echo=TRUE, results="show"}
...
$pSup = \frac{{scaling.factor.Sup \times Sup}}{{scaling.factor.Sup \times Sup + scaling.factor.Pellet \times Pellet}}$  

--------

Here function **calculate_pSup** takes the wide format data in terms of fractions and output
a data frame that includes a variable pSup calculated for each transcript. You can also specify the values of "chains", "iteration" and "control" as you prefer as the input arguments in this function. For example, `calculate_pSup(wide_data = mydata, chains = 3, iter = 1500, control = list(adapt_delta = 0.85))`.
```{r calculate_pSup, echo=TRUE, results="show"}
data_pSup <- calculate_pSup(wide_data)
knitr::kable(head(data_pSup), align="c")
```

Clear format to represent the proportion of each transcript in the Supernatant. Similar to function **model_fit**, you can also specify the values of "chains", "iteration" and "control" as you prefer. For example, `each_mRNA_pSup(wide_data = mydata, chains = 3, iter = 1500, control = list(adapt_delta = 0.85))`.
...
Function **write_results** takes the outputs of functions in RNAFracQuant as the input arguments and then writes them into text files under the same "Results" directory. You can input unlimited number of arguments in this function, for example, `write_results(data1, data2, data3, ..., data_n)`. However, all the arguments should be data frames or tibbles, which can be written into files by `readr::write_tsv`. See [write_tsv](https://readr.tidyverse.org/reference/write_delim.html) or type `?readr::write_tsv` in Rstudio for more info.
...
To identidy whether our statistical model is reliable, we plotted the calculated pSup versus the actual pSup values. We can see that the distributions of inferred and actual pSup values do perfectly match each other. Therefore, we are confident to say that our model is reliable statistically. 

![Model Evaluation Plot](system.file("vignettes", "Actual-vs-Inferred_pSup.png", package = "RNAFracQuant"))

---------

# 4. EXTRA INFO
## 4.1 Issues update
Please refer to [this page](https://github.com/thebestecho/demo_RNAFracQuant) for more info.
